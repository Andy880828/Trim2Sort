...
ref = "C:\\Users\\andy0\\Desktop\\Lab\\usearch11.0.667_win32.exe\\Trim2Sort_ver3.0.0\\ref_ver3_0408-3.xlsx"
# def Highlight(row):
...
class Sanger_ContentFrame(customtkinter.CTkFrame):
    ...
    def Analysis(self):
        ...
        merged_fastq = output_name + "_merged.fastq"
        ...
        # trim sequences
        trimmomatic_path = "C:\\Users\\andy0\\Desktop\\Lab\\程式\\Trimmomatic-0.39\\trimmomatic-0.39.jar"
        trimming_cmd = f'{trimmomatic_path} SE -phred33 {merged_fastq} {trimmed_fastq} LEADING:20 TRAILING:20'
        ...
        if self.database_combobox.get() == "Teleostei-12S":
            os.chdir("C:/Users/andy0/Desktop/Lab/NCBI/")
            blast_cmd = (
                f"blastn.exe -query {trimmed_fasta} -out {blasted_txt}"
                f" -db 12S -outfmt \"6 qseqid pident qcovs sscinames sacc\" -max_target_seqs 1"
                )
            ...
            blast_cmd = (
                f"blastn.exe -query {trimmed_fasta} -out {blasted_txt}"
                f" -db COI_Osteichthyes -outfmt \"6 qseqid pident qcovs sscinames sacc slen\" -max_target_seqs 1"
                )
            subprocess.run(blast_cmd, shell=True)

        with open(blasted_txt, 'r') as blast_result:
        ...
        blastresult = pd.read_csv(blasted_txt, header=None, sep='\t', names=['No','Identity','Coverage','Scientific_name','Accession_number', 'bp'])
