...
ref = "C:\\Users\\andy0\\Desktop\\Lab\\usearch11.0.667_win32.exe\\Trim2Sort_ver3.0.0\\ref_ver3_20240804.xlsx"
...
class NGS_ContentFrame(customtkinter.CTkFrame):
    ...
    def Blast(self,file):
        ...
        {outpath}/{file}_blasted.txt -db 12S \
    ...
    def Analyse(self):
        ...
        for n, i  in zip(range(0, len(txts)), range(1, len(reads), 2)):
            if txts[n].endswith('.txt'):
                ...
                drop_filtered = pd.merge(drop_filtered, reads_data)

                ratio = csv_data.Reads / float(total_reads)
                csv_data.insert(7, column='Ratio', value=ratio)
                final_csv = pd.concat([csv_data, drop_dupe], axis=0, ignore_index=True) 
                final_csv = final_csv.sort_values(['OTU','Stat'], ignore_index=True)
                final_csv = final_csv.groupby('OTU').apply(lambda x: x.drop_duplicates(subset=['Scientific_name'], keep='first'))
                final_csv = pd.concat([final_csv, drop_filtered], axis=0, ignore_index=True) 
